...
export default async function MyOrders({ params }: { params: Promise<{ companySlug: string }> }) {
  ...
  const orders = await prisma.order.findMany({ where: { createdById: userId }, orderBy: { createdAt: "desc" }, include: { items: true } });
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {orders.map((o) => (
          <Link key={o.id} href={`/${companySlug}/my-orders/${o.id}`} className="block rounded-xl border border-slate-200 bg-white p-5 hover:shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="font-semibold text-blue-600">{o.total} {o.currency}</p>
