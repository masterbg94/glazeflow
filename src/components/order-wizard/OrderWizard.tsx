...
export function OrderWizard({
  ...
  function applicableFor(item: CatalogItem) {
    const ak = item.applicableKinds as string[] | null | undefined;
    if (!ak || ak.length === 0) return true;
    return ak.includes(kind);
  }

  const visibleHardware = hardwareItems.filter(applicableFor);
  const visibleProcessing = processingOptions.filter(applicableFor);
      ...
      ? [
          {
            ...
            thicknessMm: pane1,
            thicknessSurchargePercentPerMm: Number(
              selectedGlass?.thicknessSurchargePercentPerMm ?? 5
            ),
          },
          ...(layers !== 'SINGLE'
            ? [
      ...
      <div className="space-y-6 lg:col-span-2">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(['FINISHED_WINDOW', 'FINISHED_DOOR', 'GLASS_ONLY', 'RAW_PROFILE'] as Kind[]).map(
              (k) => (
                <button
                  key={k}
                  onClick={() => setKind(k)}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {layers !== 'SINGLE' && (
                    <Field label="Staklo 1 (mm)">
                      <select
                        value={pane1}
                        onChange={(e) => setPane1(+e.target.value)}
                        className="input"
                      >
                  ...
                  {layers === 'DOUBLE' && (
                    <Field label="Staklo 2 (mm)">
                      <select
                        value={pane2}
                        onChange={(e) => setPane2(+e.target.value)}
                        className="input"
                      >
                        {(selectedGlass.availableThicknessMm as any)?.map((t: number) => (
                          <option key={t} value={t}>
                  ...
                  {layers === 'TRIPLE' && (
                    <>
                      ...
                      <Field label="Staklo 3 (mm)">
                        <select
                          value={pane3}
                          onChange={(e) => setPane3(+e.target.value)}
                          className="input"
                        >
                          {(selectedGlass.availableThicknessMm as any)?.map((t: number) => (
                            <option key={t} value={t}>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <div className="space-y-2">
            ...
            {visibleHardware.length === 0 && (
              <p className="text-sm text-slate-400">Nema opreme dostupne za ovaj tip proizvoda.</p>
            )}
          </div>
        </section>
          ...
          <div className="space-y-2">
            ...
            {visibleProcessing.length === 0 && (
              <p className="text-sm text-slate-400">
                Nema opcija obrade dostupnih za ovaj tip proizvoda.
              </p>
            )}
          </div>
        </section>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRequestedDate(e.target.value ? new Date(e.target.value) : null)
                    }
